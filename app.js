/* =============================================================
   Starcya — Core Application Logic
...
(function () {
    ...
    function generatePortfolioHTML(info, projects, theme, inline) {
        ...
        return `<!-- Portfolio generated by Starcya | (c) 2026 Raicy. All Rights Reserved. -->
<!DOCTYPE html>
...
<head>
    ...
    <link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='${encodeURIComponent(theme.primaryColor)}' stroke='${encodeURIComponent(theme.primaryColor)}' stroke-width='0.5'%3E%3Cpolygon points='12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2'/%3E%3C/svg%3E">
    ${styleTag}
</head>
<body>
    ...
    <footer class="footer">
        <div class="container">
            ...
            <div class="footer-bottom">
                ...
                <p class="footer-credit">Built with <a href="https://r41cy.github.io/Auto-Portfolio-Generator/" target="_blank" rel="noopener">Starcya</a></p>
            </div>
        </div>
    </footer>
    ...
    function generatePortfolioCSS(theme) {
        ...
        return `/* Auto-generated by Starcya */
...
.hero-photo-col {
    ...
    opacity: 0; animation: fadeUp .9s cubic-bezier(.16,1,.3,1) .1s forwards;
}
.hero-photo-col::after {
    ...
    opacity: 0.5; transition: opacity .5s ease, top .5s ease, left .5s ease;
}
.hero-photo-col:hover::after { opacity: 0.8; top: 16px; left: 16px; }
[data-theme="light"] .hero-photo-col::after { opacity: 0.35; }
[data-theme="light"] .hero-photo-col:hover::after { opacity: 0.55; }
.hero-photo-frame {
...
.avail-line { width: 28px; height: 2px; background: var(--primary); border-radius: 2px; flex-shrink: 0; }
...
@media (max-width: 768px) {
    ...
    .hero-photo-col::after { top: 10px; left: 10px; right: -10px; bottom: -10px; }
...
@media (max-width: 420px) {
    ...
    .hero-photo-col::after { top: 8px; left: 8px; right: -8px; bottom: -8px; }
...
(function(){
    ...
    function initStarCanvas() {
        ...
        function draw() {
            ...
            stars.forEach(function (s) {
                const tw = Math.sin(t * s.speed * 50 + s.phase) * 0.4 + 0.6;
                ctx.beginPath();
                ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2);
                ctx.fillStyle = 'rgba(251,191,36,' + (s.alpha * tw).toFixed(3) + ')';
                ctx.fill();
                if (s.r > 1) {
                    ctx.beginPath();
                    ctx.arc(s.x, s.y, s.r * 3, 0, Math.PI * 2);
                    ctx.fillStyle = 'rgba(251,191,36,' + (s.alpha * tw * 0.08).toFixed(4) + ')';
                    ctx.fill();
                }
            });
    ...
    if (landing) {
        if (document.documentElement.classList.contains('app-ready')) {
            // Returning user — landing already hidden via CSS
        } else {
            // First-time user — show star canvas + handle CTA
            const stopCanvas = initStarCanvas();
            if (landingCta) {
